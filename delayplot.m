%% Plot time delay embedded(2D) representation of a time series.
%
%  Exploratory tool for determining(getting a feel for) suitable time delay to
%  use in the embedding(2D) of a given time-series. Delay is constant between
...
%{
    ...
    X = [sin(0:(2*pi/42):(2*pi));                           ...
         ...
         sin(0:(2*pi/42):(2*pi)) .* cos(0:(2*pi/42):(2*pi))].';
    % Plot time delay embedded(2D) representations
    delayplot(X);
...
function [figure_handle, axes_handle] = delayplot(x, delay_range, colourmaps)
    ...
    % Lazy axes creation
    dummy_scatter = scatter(standardise_range(rand(1,42), [AxisMin, AxisMax]), ...
    ...
    for delay = delays
        ...
        for k = 1:size(x, 2)
            ...
            cmap = trajectory_colourmap(cmap_func(tpts - delay));
